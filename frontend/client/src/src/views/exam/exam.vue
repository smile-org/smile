  ...
  <div id="app">
      ...
      <div class="course_tit mt3 subject_list mb3hafe">
        ...
        <dl class="">
          ...
          <dd v-for="item in currentQuestion.answers" :key="item.anwser_id">
            <span class="choose_a" v-on:click="choose(item)"
                  v-bind:class="{c_g : item.selected}">{{item.answer_option}}</span> {{item.answer_content}}
          </dd>
        </dl>
        <img src="../../assets/img/cy_img.png" v-if="willShow" class="cy_img imgAnimation" style="">
      </div>
    </section>
    <footer>
      <el-row class="exam_b">
        ...
        <el-col class="btn" v-bind:class="{active: currentQuestionNo === questionCount}" v-on:click="goNext" :span="8">
          <span :disabled="currentQuestionNo === questionCount"
                v-bind:class="{active: currentQuestionNo === questionCount}" v-on:click="goNext">下一题</span>
        </el-col>
      </el-row>
    ...
    <div class="popup_box" v-show="showModal">
      <div v-if="popupType === 0">
        ...
        <ul class="q_num">
          <li v-on:click="gotoQuestion(index)" v-for="(item, index) in data.questions" :key="item.question_id"
              :class="{c_o: item.lastStatus === 'indoubt', c_g: item.lastStatus === 'finished'}">{{index + 1}}
          </li>
        </ul>
        <div class="el-row dio_btn mt3">
          ...
          <div class="el-col el-col-12">
            <span class="c_g btn fl" v-on:click="submitExam">交卷</span>
          </div>
        </div>
      </div>
      ...
      <div v-else>
        ...
        <ul class="q_num">
          <li v-for="(item, index) in data.questions" :key="item.question_id"
              :class="{c_o: item.lastStatus === 'indoubt', c_g: item.lastStatus === 'finished'}">{{index + 1}}
          </li>
        </ul>
        <div class="el-row dio_btn mt3">
...
//            _self.currentQuestionNo = val
            if (!_self.currentQuestion.lastStatus || _self.currentQuestion.lastStatus !== 'indoubt') {
              _self.willShow = false
            } else {
              _self.willShow = true
            }
          } else {
      ...
      goPre: function () {
        console.log(this.currentQuestion)
//        if (!this.currentQuestion.lastStatus || this.currentQuestion.lastStatus !== 'pending') {
//          this.willShow = false
//        } else {
//          this.willShow = true
//        }
            ...
            api.fetch(api.uri.finishExam, {historyid: __self.historyId, examid: __self.id}).then(data => {
              ...
              if (data.status === 1) {
                if (data.result.status === 'pass') {
                  router.push({
                    name: 'examSuccess',
                    query: {score: data.result.score, id: data.result.exam_id, historyId: data.result.history_id}
                  })
                } else {
                  router.push({
                    name: 'examFailed',
