      ...
      api.fetch(api.uri.getExam, {examid: this.id}).then(data => {
        if (data.status === 1) {
          ...
          this.form = {
            ...
            single: currentExam.single_choice_score,
            multi: currentExam.multi_choice_score,
            truefalse: currentExam.true_false_score,
            totalScore: currentExam.total_score
          }
